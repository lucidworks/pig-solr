...
public class SolrStoreFuncTest {
  ...
  public void testBadTuple() throws Exception {
    ...
    //TODO: add test for null input
    /*try {
      PigStatusReporter instance = PigStatusReporter.getInstance();
      Configuration conf = new Configuration();
      PigStatusReporter.setContext(new MapContext(conf, new org.apache.hadoop.mapred.TaskAttemptID(), new BinStorageRecordReader(), new BinStorageRecordWriter(new DataOutputBuffer()), new FileOutputCommitter(),
              new TestStatusReporter(), new FileSplit(new Path("."), 0, 0, new String[]{})));
      Counter counter = instance.getCounter(SolrStoreFunc.WARNINGS.NULL_ID);
      func.putNext(createTuple(null, "odd", "even", "odd2", "even2", "odd3", "even3"));
      fail();
    } catch (IOException e) {
...
//    Assert.assertEquals(4, nestFields.size());
//    for (int i = 0; i < 4; i++) {
//      Assert.assertEquals(String.valueOf(i + 1), nestFields.get(i).getValue().toString());
//    }
//    nestFields = doc.getFields("nest2");
//    Assert.assertEquals(6, nestFields.size());//has all sublevels too
//    nestFields = doc.getFields("nest3");
//    Assert.assertEquals(9, nestFields.size());//has all sublevels too
//    nestFields = doc.getFields("databag");
//    Assert.assertEquals(4, nestFields.size());//has all sublevels too
  }
