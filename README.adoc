:jarfilename: solr-pig-functions-<version>.jar
...
solr-pig-functions/build/libs/{jarfilename}
...
The Pig functions included in the `{jarfilename}` are three UserDefined Functions (UDF) and one Store function. These functions are:
...
----
./bin/pig -Dpig.additional.jars=/path/to/{jarfilename} -p csv=/path/to/my/csv/airports.dat -p zkHost=zknode1:2181,zknode2:2181,zknode3:2181/solr -p collection=myCollection ~/myScripts/index-csv.pig
----
