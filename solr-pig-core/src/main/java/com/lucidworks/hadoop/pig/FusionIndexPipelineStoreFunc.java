...
public class FusionIndexPipelineStoreFunc extends StoreFunc {
  ...
  public void putNext(Tuple input) throws IOException {
    ...
    if (fieldNamesFromSchema == null) {
      Properties udfProps = UDFContext.getUDFContext().getUDFProperties(this.getClass(), new
          String[]{myUDFContextSignature});
      ...
      if (fieldNamesFromSchema == null || fieldNamesFromSchema.isEmpty()) {
        throw new IOException("No field names passed to backend from front-end schema processing! Expected property "
            + FIELD_NAMES_FROM_SCHEMA_PROPS_KEY);
      }
    }
    ...
    if (input.size() != fieldNamesFromSchema.size()) {
      ...
      log.error(String.format("Failed to convert Tuple %s to JSON object due to schema mis-match! Expected %d fields " +
              "but input has %d",
    ...
    Callable<Object> task = new Callable<Object>() {
      public Object call() {
        try {
          ...
          log.error("{In java.util.concurrent.Callable} Failed to send batch after " + timeoutSecs + " secs due to: "
              + exc.toString());
  ...
  protected void handleSolrServerException(Tuple input, Exception e) throws IOException {
    ...
    if (batchSize > 1) {
      ...
      log.error(String.format("Failed to add batch containing %d inserts due to: %s", count, rootCause.getMessage()),
          rootCause);
  ...
  class SolrOutputFormat extends OutputFormat<NullWritable, NullWritable> {
    ...
    @Override
    public RecordWriter<NullWritable, NullWritable> getRecordWriter(TaskAttemptContext context) throws IOException,
      ...
      return new RecordWriter<NullWritable, NullWritable>() {
        ...
        public void close(TaskAttemptContext context) {
          if (FusionIndexPipelineStoreFunc.this.batch != null && FusionIndexPipelineStoreFunc.this.batch.size() > 0) {
            log.info("In getRecordWriter.close(), committing write to FusionIndexPipelineStoreFunc task " + context
                .getTaskAttemptID());
            // we have one final batch to commit
            try {
              log.info("In getRecordWriter.close(), executing final batch of size " + FusionIndexPipelineStoreFunc
                  .this.batch.size());
              if (sendBatchWithTimeout(batch, 600)) {
                incrementCounter(Counters.NUM_DOCS_INDEXED, FusionIndexPipelineStoreFunc.this.batch.size());
              }
  ...
  public void prepareToWrite(RecordWriter writer) throws IOException {
    try {
      pipelineClient = fusionAuthEnabled
          ? new FusionPipelineClient(Reporter.NULL, endpoints, fusionUser, fusionPass, fusionRealm)
          : new FusionPipelineClient(Reporter.NULL, endpoints);
    } catch (Exception e) {
